		    ...
		    echo '</tr>';
		      ...
		      if( mysqli_num_rows( $selectRes )==0 ){
		        ...
		        while( $row = mysqli_fetch_assoc( $selectRes ) ){
		          echo "<tr><td>{$row['p_id']}</td><td>{$row['productName']}</td><td>{$row['color']}</td><td>{$row['Price']}</td><td><a href='update.php?q={$row['p_id']}'>Update</a></td><td><a href='delete.php?q={$row['p_id']}'>Delete</a></td></tr>\n";
		        }
		      }
			echo '</table>'; 
			}
